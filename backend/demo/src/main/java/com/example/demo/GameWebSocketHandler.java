...
import com.fasterxml.jackson.databind.JsonNode; // ¡IMPORTANTE: Asegúrate de importar JsonNode!
import com.fasterxml.jackson.databind.node.ObjectNode; // Importar ObjectNode si se usa para construir JSON
...
public class GameWebSocketHandler extends TextWebSocketHandler {
    ...
    // ¡DECLARACIÓN DE 'games' Y OTRAS COLECCIONES!
    ...
    private static class Player {
        ...
        double y;
        int score;
        // int playerId; // Este campo no es necesario si usas session.getId() como ID único
        // String playerKey; // Podrías almacenar el rol ('Sighttail', 'Scentpaw') aquí también,
    ...
    private static class Game {
        ...
        // Posiciones iniciales para los jugadores en esta partida (pueden ser dinámicas)
        ...
        // Estado del juego (ej: posición del cuadrado collectible, si aplica para la GameScene real)
        double squareX;
        double squareY;
        ScheduledFuture<?> timerTask;
        long startTime;
        int totalTime = 60000; // 60 seconds
    ...
    public void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        JsonNode rootNode = mapper.readTree(payload); // ¡DECLARACIÓN DE 'rootNode'!
        ...
        switch (type) {
            case "player_join":
                System.out.println("Solicitud de 'player_join' de: " + session.getId());
                if (!waitingPlayers.isEmpty()) {
                    ...
                    if (opponentPlayer != null) {
                        ...
                        Game newGame = new Game(gameId, currentPlayer, opponentPlayer); // currentPlayer será player1 (Sighttail)
                                                                                  // opponentPlayer será player2 (Scentpaw)
                        
                        // Asignar los roles y ID de la partida a los jugadores
                        // Guardar la partida en el mapa 'games' usando el sessionId de ambos jugadores como clave
                        ...
                        System.out.println("Partida creada: " + gameId + " entre " + currentPlayer.session.getId() + " (Sighttail) y " + opponentPlayer.session.getId() + " (Scentpaw)");
            ...
            case "player_move":
                Game game = games.get(session.getId()); // Obtener el objeto Game
                if (game != null) {
                    double x = rootNode.get("x").asDouble();
                    double y = rootNode.get("y").asDouble();
                    String anim = rootNode.get("anim").asText();
                    boolean flipX = rootNode.get("flipX").asBoolean();

                    // Actualizar la posición del jugador en el servidor (opcional, pero buena práctica)
                    currentPlayer.x = x;
                    currentPlayer.y = y;

                    // Determinar cuál es el otro jugador en la partida
                    Player otherPlayer = (currentPlayer == game.player1) ? game.player2 : game.player1;
            ...
            case "ability_use":
                ...
                if (gameAbility != null) {
                    String ability = rootNode.get("ability").asText();
                    Player otherPlayer = (currentPlayer == gameAbility.player1) ? gameAbility.player2 : gameAbility.player1;
                    sendToPlayer(otherPlayer, "a", Map.of("playerId", currentPlayer.session.getId(), "ability", ability)); // 'a' de ability
                }
                break;
            ...
            case "door_interact":
                ...
                if (gameDoor != null && !gameDoor.doorOpened) { // Solo si la puerta no ha sido interactuada ya
                    String playerKey = rootNode.get("playerKey").asText();
                    ...
                    // ¡Aquí el servidor decide cuándo activar el agujero y las habilidades del tutorial!
                    if (!gameDoor.agujeroVisible) {
                        gameDoor.agujeroVisible = true;
                        // Enviar un mensaje específico para que ambos clientes hagan visible el agujero (type 'g', progressType)
                        sendToPlayer(gameDoor.player1, "g", Map.of("progressType", "agujero_visible", "message", "El agujero ha aparecido."));
                        sendToPlayer(gameDoor.player2, "g", Map.of("progressType", "agujero_visible", "message", "El agujero ha aparecido."));
                    }
                    if (!gameDoor.tutorialAbilitiesActivated) {
                        gameDoor.tutorialAbilitiesActivated = true;
                        // Enviar un mensaje específico para que ambos clientes activen las habilidades (type 'g', progressType)
                        sendToPlayer(gameDoor.player1, "g", Map.of("progressType", "abilities_activated", "message", "Tus habilidades están activadas."));
                        sendToPlayer(gameDoor.player2, "g", Map.of("progressType", "abilities_activated", "message", "Tus habilidades están activadas."));
                    }
                }
                break;

            case "agujero_interact":
                ...
                if (gameAgujero != null) {
                    ...
                    System.out.println(playerKey + " interacted with agujero. State: Sighttail=" + gameAgujero.sighttailInteractedWithAgujero + ", Scentpaw=" + gameAgujero.scentpawInteractedWithAgujero + " en partida " + gameAgujero.gameId);
                    ...
                    if (gameAgujero.sighttailInteractedWithAgujero && gameAgujero.scentpawInteractedWithAgujero) {
                        System.out.println("Ambos jugadores interactuaron con el agujero en partida " + gameAgujero.gameId + ". Notificando a clientes para avanzar.");
                        // Enviar un mensaje para CAMBIAR DE ESCENA (type 'g', progressType)
                        sendToPlayer(gameAgujero.player1, "g", Map.of("progressType", "agujero_ambos_interactuaron", "message", "Ambos interactuaron con el agujero. Avanzando a GameScene."));
                        sendToPlayer(gameAgujero.player2, "g", Map.of("progressType", "agujero_ambos_interactuaron", "message", "Ambos interactuaron con el agujero. Avanzando a GameScene."));
                        
                        // Resetear el estado de interacción (opcional, si el agujero es de un solo uso por partida)
    ...
    public void endGame(Game game) {
        if (game == null) return; // Add null check for robustness

        // Notify players that the game has ended
        Map<String, String> finalScores = Map.of("message", "Game Ended!"); // You can add scores here if your game has them

        // Check if sessions are still open before sending messages
        if (game.player1.session.isOpen()) {
            sendToPlayer(game.player1, "o", finalScores); // 'o' for game_over
        }
        if (game.player2.session.isOpen() && !game.player1.session.getId().equals(game.player2.session.getId())) {
        ...
        // Remove games using player session IDs to avoid ConcurrentModificationException if iterating
        // Use a temp set to avoid issues if player1 and player2 IDs are the same (e.g. self-connection error)
        Set<String> sessionIdsToRemove = new HashSet<>();
        if (game.player1 != null) sessionIdsToRemove.add(game.player1.session.getId());
        if (game.player2 != null) sessionIdsToRemove.add(game.player2.session.getId());
    ...
    private void sendToPlayer(Player player, String type, Object data) {
        try {
            // Asegurarse de que el objeto de datos sea un JSON válido o crear uno vacío si es null
            ObjectNode jsonNode;
            if (data instanceof Map) {
                ...
                // Si data no es un Map y no es null, se asume que es un objeto que ObjectMapper puede serializar.
